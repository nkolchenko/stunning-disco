...
print("Words: ",len(my_list))
while incr < len(my_list):

    for element in my_list:

        cnt=my_list.count(my_list[incr])
        print("word",incr+1," : ", my_list[incr], " found ", cnt," times" )

        my_dict.update({my_list[incr]:cnt})
        incr = incr + 1
